...
CDANN_WARMUP_START = 20  # R < this: L_dom_* fully disabled (true baseline equivalence)
CDANN_WARMUP_END = 40    # R >= this: lambda_adv = 1.0 (full CDANN)


def compute_lambda_adv(current_round: int,
                       warmup_start: int = CDANN_WARMUP_START,
                       warmup_end: int = CDANN_WARMUP_END) -> float:
    """3-stage lambda_adv schedule (standard DANN practice, R4 settled).

    R=0..warmup_start : 0 (return 0; CALLER MUST ALSO SKIP L_dom_sem + L_dom_sty
                          completely to be true baseline-equivalent; otherwise
                          dom_head + style_head still get updated from round 0).
...
class Server(flgo.algorithm.fedbase.BasicServer):
    def initialize(self):
        ...
        # Codex IMPORTANT fix: ca=1 和 ue=1 的组合语义冲突 (Fixed ETF + CDANN 混用未设计).
        # 明确 raise, 避免 silent 运行产生难以解释的结果.
        if int(self.ca) == 1 and int(self.ue) == 1:
            raise ValueError(
                "[CDANN config] ca=1 requires ue=0 (Linear classifier). "
                "Got ca=1 + ue=1. See FINAL_PROPOSAL.md: CDANN 设计基于 Linear head."
            )

        # Codex IMPORTANT fix: dom_head 输出维度可能和 runtime len(clients) 不匹配
        # (task name heuristic _TASK_NUM_CLIENTS 不一定准). Server.initialize 时 clients
        # 已就绪, 这里做一次动态验证/重建, 确保 dom_head.out = len(self.clients).
        if int(self.ca) == 1 and self.model.dom_head is not None:
            actual_N = len(self.clients)
            out_dim = self.model.dom_head[-1].out_features
            if out_dim != actual_N:
                print(f"[CDANN] dom_head rebuild: out_dim {out_dim} -> {actual_N} "
                      f"(task heuristic mismatch with runtime clients)")
                device = next(self.model.parameters()).device
                self.model.dom_head = nn.Sequential(
                    nn.Linear(self.pd, 64),
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.1),
                    nn.Linear(64, actual_N),
                ).to(device)
                self.model.num_clients = actual_N
...
class Client(flgo.algorithm.fedbase.BasicClient):
    ...
    def train(self, model, *args, **kwargs):
        ...
        for step in range(num_steps):
            ...
            #
            # Codex CRITICAL fix: warmup 阶段 (R < CDANN_WARMUP_START) **完全跳过**
            # L_dom_sem + L_dom_sty (dom_head 和 style_head 也不被更新), 保证与 baseline
            # 严格等价. 单独 lambda_adv=0 只能禁 encoder z_sem 路径, 不够.
            # 见 refine-logs/2026-04-20_FedDSA-CDANN/FINAL_PROPOSAL.md
            ...
            cdann_active = (ca_flag == 1 and model.dom_head is not None
                            and self.current_round >= CDANN_WARMUP_START)
            if cdann_active:
                lambda_adv_val = compute_lambda_adv(
                    self.current_round, CDANN_WARMUP_START, CDANN_WARMUP_END)
                B = z_sem.size(0)
                # Domain label = client id. Bound check: id 必须在 [0, num_clients).
                # Codex IMPORTANT fix: 防止 task heuristic 和 runtime len(clients) 不匹配
                cid = int(getattr(self, 'id', 0) or 0)
                out_dim = model.dom_head[-1].out_features
                if cid >= out_dim:
                    raise RuntimeError(
                        f"client.id={cid} >= dom_head out_dim={out_dim}. "
                        f"Check _TASK_NUM_CLIENTS matches runtime len(clients)."
                    )
                d_label = torch.full((B,), cid, dtype=torch.long, device=x.device)
