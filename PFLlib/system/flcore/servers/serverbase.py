...
class Server(object):
    ...
    # 选择客户端，按照join_ratio或random_join_ratio选择客户端
    def select_clients(self):
        # 随机选择客户端，如果self.random_join_ratio为True
    ...
    def receive_models(self):
        ...
        for client in active_clients:
            try:
                ...
                client_time_cost = 0
            # 如果客户端时间成本小于阈值，则上传客户端，慢客户端的淘汰机制
            if client_time_cost <= self.time_threthold:
                # 累加总样本数
                tot_samples += client.train_samples
                # 上传客户端ID
                self.uploaded_ids.append(client.id)
                # 上传客户端权重，使用样本数量来表示上传客户端的权重
