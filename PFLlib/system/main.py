...
from flcore.servers.serverdsa import FedDSA
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif args.algorithm == "FedDSA":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedDSA(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-fdim', "--feat_dim", type=int, default=512,
                        help="Backbone feature dimension")
    parser.add_argument('-pdim', "--proj_dim", type=int, default=128,
                        help="Projection head output dimension")
    parser.add_argument('-lo', "--lambda_orth", type=float, default=1.0,
                        help="Weight for orthogonal decoupling loss")
    parser.add_argument('-lh', "--lambda_hsic", type=float, default=0.1,
    ...
    parser.add_argument('-sdn', "--style_dispatch_num", type=int, default=5,
                        help="Number of style prototypes dispatched per client")
    parser.add_argument('-sdd', "--style_dedup", type=float, default=0.95,
