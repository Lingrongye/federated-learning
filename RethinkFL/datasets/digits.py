...
class MyDigits(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        if self.data_name == 'mnist':
            dataobj = MNIST(self.root, self.train, None, None, self.download)
        elif self.data_name == 'usps':
            dataobj = USPS(self.root, self.train, None, None, self.download)
        elif self.data_name == 'svhn':
            if self.train:
                dataobj = SVHN(self.root, 'train', None, None, self.download)
            else:
                dataobj = SVHN(self.root, 'test', None, None, self.download)
        return dataobj
...
class FedLeaDigits(FederatedDataset):
    ...
    percent_dict = {'mnist': 0.01, 'usps': 0.01, 'svhn': 0.01}
