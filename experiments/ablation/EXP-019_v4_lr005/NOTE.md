...
| 指标 | 值 |
|------|---|
| Best acc | **79.90%** @Round 55 |
| Last acc | 77.98% |
| **Drops>5%** | **2** (最少) |
| 最弱域@best | 67.16% |
| 最强域@best | 87.76% |
| 稳定性 | 0.0036 |
| vs EXP-014 (lr=0.1) | -0.03% Best，**更稳** |
| 总轮次 | 200 |

## 结论

**学习率0.05验证结果**：
- Best几乎相同(79.90% vs 79.93%)，说明lr对上限影响小
- **Drops最少(只有2次)** ——学习率降低确实显著提高稳定性
- Last 77.98%最接近Best——过拟合减少

**建议**：如果最终配置用EXP-017(V4去HSIC)，可以考虑同时把lr改为0.05进一步提升稳定性。
但收敛速度会变慢，需要更多轮次。
