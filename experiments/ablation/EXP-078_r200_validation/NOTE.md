...
| 333 | 102 | 15.9 | **15.0** | **-0.9** | ⚠️ R2就崩溃，NaN loss锁死，从未恢复 |
| 42 | 102 | **79.1** | 78.5 | -0.6 | 稳定但不如orth_only |
| **Mean** (去除s=333) | | **80.4** | **78.8** | **-1.6** | |

> ⚠️ 注意：s=333 的旧表中"max=60.2%"是误读。实际上 s=333 从 R2 就因 NaN loss 崩溃至 15.04%，完全没有有效训练。
>
> ❌ detach_aug 结论：CE 只用原始特征、InfoNCE 用增强特征 → 两条梯度流分离 → 在敏感 seed 下从开始就 NaN；在稳定 seed 下峰值可达 81.7%，但 R100+ 缓慢下滑至 78-79%，最终不如 orth_only。
...
2. **078d detach_aug 不稳定且无增益** — s=333 从 R2 NaN崩溃；稳定 seed 峰值 80-81% 但 R100+ 滑落 78-79%，不超过 orth_only 80.7%；主动终止
3. **078c (mse_alpha R200，lab-lry)** — lab-lry 连接失败，结果未知；参考 R50 验证推测长期也会失效
4. **Office 验证 orth_only** — 两数据集均有效，正交解耦是通用机制
5. **当前最可靠方案 = orth_only** — PACS 80.7%（超FDSE 80.36%），Office 88.5%（超FedBN 88.65%）
6. **InfoNCE / 安全阀思路在 R200 全面告失** — 所有安全阀变体（MSE锚点/alpha-sparsity/detach）在 R200 均不超过 orth_only，且均不稳定
7. **状态**: ⛔ 078a 继续运行(SC2)，078c 未知(lab-lry)，**078d 主动终止(SC4)**
