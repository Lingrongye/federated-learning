...
## 🏆 结果（R200 完整，2026-04-17 23:11 完成）

### Office 方案 A (sas=1, sas_tau=0.3) 3-seed (2/15/333)

| seed | ALL Best | ALL Last | AVG Best | AVG Last | best@R |
|------|---------|---------|---------|---------|-------|
| 2 | 81.76 | 80.17 | 88.61 | 87.64 | R61 |
| 15 | 84.14 | 82.15 | 90.11 | 88.16 | R42 |
| 333 | 87.31 | 86.89 | 90.74 | 89.03 | R149 |
| **mean** | **84.40** | **83.07** | **89.82** | **88.28** | — |

### 🎯 同 baseline 对比 (EXP-083 相同训练脚本，仅 sas=0 → 1)

**3-seed mean**：

| 指标 | baseline (sas=0) | 方案 A (sas=1) | Δ |
|------|-----------------|----------------|---|
| ALL Best | 82.55 | **84.40** | **+1.85** ★ |
| ALL Last | 81.35 | **83.07** | **+1.72** ★ |
| AVG Best | 88.61 | **89.82** | **+1.21** ★ |
| AVG Last | 87.30 | **88.28** | **+0.98** ★ |

**Per-seed Δ (方案A - baseline)**：

| seed | ΔAVG Best | ΔAVG Last | ΔCaltech Best | ΔCaltech Last |
|------|----------|----------|--------------|--------------|
| 2 | +2.17 | +1.20 | +4.5 | +2.7 |
| 15 | +0.53 | +0.41 | -0.9 | **+2.7** |
| 333 | +0.93 | +1.34 | +3.6 | **+5.4** ★ |

### Per-domain AVG Best/Last (关键 — Caltech 是否提升?)

| 方法 | Caltech | Amazon | DSLR | Webcam |
|------|---------|---------|------|--------|
| FDSE (EXP-051) | 78.9/77.7 | 92.3/89.5 | 100.0/97.8 | 95.4/92.0 |
| baseline sas=0 (EXP-083) | 72.6/70.2 | 90.9/88.1 | 100.0/97.8 | 94.3/93.1 |
| **+sas 方案 A (EXP-084)** | **75.0/73.8** | 91.6/88.4 | 100.0/97.8 | 95.4/92.8 |
| **Δ vs baseline** | **+2.4/+3.6** ★ | +0.7/+0.3 | 0/0 | +1.1/-0.3 |

### vs FDSE (gap 收窄)

| 指标 | baseline | 方案 A | FDSE | A 距离 FDSE |
|------|---------|-------|------|------------|
| AVG Best | 88.61 | **89.82** | 90.58 | **-0.76** |
| AVG Last | 87.30 | **88.28** | 89.22 | **-0.94** |
| Caltech Best | 72.6 | **75.0** | 78.9 | -3.9 (仍有 gap) |

## ✅ 核心结论

1. **方案 A 有效** — 3-seed mean AVG Best **+1.21%**，AVG Last **+0.98%** vs 同 baseline
2. **Caltech 提升 +2.4/+3.6** — 验证了"style outlier 域受益于 personalized aggregation"的假设
3. **与 FDSE gap 从 -1.97% 缩到 -0.94%**（Office AVG Last）
4. **未引入梯度冲突**：训练稳定（方案 A 仍用 LR=0.05）

## 下一步

1. **扩展到 PACS**：看 Art（另一个 style outlier 域）是否同样受益
2. **Ablation sas_tau**：{0.1, 0.3, 1.0} 看敏感性
3. **Caltech 与 FDSE 仍差 3.9%**：可能需配合方案 B（原型推理）进一步缩小
4. 如果 PACS 上 Art 提升 → 论文核心贡献确立
