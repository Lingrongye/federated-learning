...
### Neural Collapse 诊断演进 (3-seed mean, 从 Layer 1/2 jsonl 提取)

| Round | etf_align mean | inter_cls_sim | intra_cls_sim | z_sem_norm | z_sty_norm | client_center_var | param_drift |
|-------|---------------|---------------|---------------|-----------|-----------|-------------------|-------------|
| R5 | 0.0012 | 0.3502 | 0.6714 | 8.195 | 2.102 | 0.02472 | 0.18602 |
| R50 | **0.8122** | -0.0812 | 0.8383 | 3.061 | 2.380 | 0.00164 | 0.00351 |
| R100 | **0.8545** | -0.1038 | 0.8682 | 2.765 | 2.331 | 0.00139 | 0.00233 |
| R200 | **0.8765** | -0.0941 | 0.8648 | 2.773 | 2.288 | 0.00110 | 0.00220 |

**诊断解读 (SGPA vs Linear 对比)**:
- **etf_align R200 = 0.8765** (高对齐 ETF 顶点) vs Linear R200 = 0.3561 → ETF 确实把 z_sem 压到顶点方向 ✅
- **inter_cls_sim R200 = -0.0941** vs Linear -0.0409 → SGPA 类间分离更好 ✅
- **z_sem_norm R200 = 2.77** vs Linear 12.21 → **SGPA 把 z_sem norm 压了 4.4 倍** (被 ETF 约束压缩)
- **client_center_var R200 = 0.0011** → 跨 client 类中心一致性高
- **param_drift R200 = 0.00220** → 聚合稳定

**但 accuracy 反低**: SGPA AVG Best 86.97 < Linear 88.75 (**-1.78pp**). 几何上 SGPA "好"但分类上"差" → Fixed ETF 的几何约束和真实类中心分布**不匹配**.
