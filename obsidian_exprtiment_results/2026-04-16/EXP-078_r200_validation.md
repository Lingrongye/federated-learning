...
1. **078a MSE anchor 长期失效** — R50 cos_sim=+0.678 但 R120 仍然 drop -3% 到 -6.5%。全局原型漂移，锚点随之漂移，并非静止引力中心。

2. **078d detach_aug 不稳定且无增益** — s=333 从 R2 NaN崩溃；稳定 seed 峰值 80-81% 但 R100+ 滑落至 78-79%，**不超过 orth_only 80.7%**；主动终止于 R102。

3. **078c (mse_alpha R200，lab-lry)** — lab-lry 连接失败，结果未知；参考 R50 验证推测长期也会失效。

4. **Office 验证 orth_only 有效性** — 两数据集均有效，正交解耦是通用机制。

5. **当前最可靠方案 = orth_only** — PACS 80.7%（超FDSE 80.36%），Office 88.5%（超FedBN 88.65%）。

6. **InfoNCE / 安全阀思路在 R200 全面告失** — 所有安全阀变体（MSE锚点/alpha-sparsity/detach）在 R200 均不超过 orth_only，且均不稳定。需要更根本的解决方案。

7. **状态**: ⛔ 078d 主动终止 | 🔄 078a 继续运行(SC2) | ❓ 078c 未知(lab-lry)
