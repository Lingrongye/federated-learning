...
> ⚠️ R50 时 cos_sim=+0.678（最高），R122 时仍然在下降 -3% 到 -6.5%。MSE 锚点修复了短期梯度方向，但全局原型本身也在漂移，锚点随之漂移，长期仍然失效。

### 078c MSE + Alpha (Lab-lry) — 未知 (lab-lry 连接失败)
...
> ❌ s=333 灾难性崩溃: 60.2% → 15%。detach_aug 使 CE 和对比学习看到不同的特征流，在某些 seed 下训练完全失去一致性。
...
1. **078a MSE anchor 长期失效** — R50 cos_sim=+0.678 但 R120 仍然 drop -3% 到 -6.5%。根因在于全局原型在漂移，锚点随之漂移，并非静止的引力中心。

2. **078d detach_aug 存在严重 seed 不稳定性** — s=333 灾难性崩溃 (-45%)，该模式不适合作为最终方案。

3. **Office 验证 orth_only 有效性** — 两数据集均优于纯 CE 基线，正交解耦是通用机制。

4. **当前最可靠方案 = orth_only** — CE + L_orth，无 InfoNCE，无增强。PACS mean final 80.7%（超FDSE），Office mean final 88.5%（超FedBN，接近FedDSA）。

5. **078c (mse_alpha R200) 结果待定** — 若 lab-lry 恢复后结果也失败，则整个"安全阀"思路在 R200 均告失败，需要更根本的解决方案。
