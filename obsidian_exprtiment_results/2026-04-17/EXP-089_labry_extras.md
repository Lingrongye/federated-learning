...
- **日期**: 2026-04-18
- **服务器**: Lab-lry GPU 1
- **状态**: ❌ 全部 SIGKILL 放弃（Lab-lry 被 wjc 抢 GPU）
...
❌ Lab-lry SIGKILL，未完成。不再补跑（3-seed {2,15,333} 已足够，FDG 领域惯例）。

### Office 方案 A s=42

❌ Lab-lry SIGKILL，未完成。不再补跑（3-seed 已够）。

### PACS orth_only LR=0.05 s=4388

❌ Lab-lry SIGKILL，未完成。5-seed 对齐非必须（3-seed 是 FDG 标准）。
