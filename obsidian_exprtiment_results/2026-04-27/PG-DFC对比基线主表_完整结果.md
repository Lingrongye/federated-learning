...
### Table 1b. PACS — EXP-135 完整 4-method × 2-seed R100 数据 (2026-04-29 回填, sc 高质量诊断版)
...
| Method         | seed |   photo   |    art    |  cartoon  |  sketch   |  **AVG_B**  |  **AVG_L**  | gap  |
| -------------- | :--: | :-------: | :-------: | :-------: | :-------: | :---------: | :---------: | :--: |
| F2DC vanilla   |  15  |   74.25   |   60.05   |   83.12   | **82.42** |    74.96    |    69.78    | 5.19 |
| F2DC vanilla   | 333  |   70.06   |   60.05   |   77.14   |   82.42   |    72.42    |    68.96    | 3.46 |
| F2DC+DaA       |  15  |   77.54   |   65.93   |   80.77   |   76.94   | **75.29** ⭐ | **72.97** ⭐ | 2.32 |
| F2DC+DaA       | 333  | **76.05** |   58.58   |   74.79   |   64.59   |    68.50    |    65.26    | 3.24 |
| PG-DFC vanilla |  15  |   72.16   |   59.80   | **83.97** |   81.78   |    74.43    |    71.36    | 3.07 |
| PG-DFC vanilla | 333  |   70.36   |   56.13   |   75.64   |   80.00   |    70.53    |    66.66    | 3.87 |
| PG-DFC+DaA     |  15  |   76.65   | **64.22** |   80.98   |   69.94   |    72.95    |    69.26    | 3.68 |
| PG-DFC+DaA     | 333  |   73.05   |   61.27   |   75.85   |   70.19   |    70.09    |    66.88    | 3.21 |

#### 2-seed mean 主表 (核心对比)

| Method | photo | art | cartoon | sketch | **AVG_B** | **AVG_L** | gap |
|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 🥇 **F2DC vanilla** | 72.15 | 60.05 | 80.13 | **82.42** ⭐ | **73.69** | 69.37 | 4.32 |
| 🥈 PG-DFC vanilla | 71.26 | 57.97 | 79.81 | 80.89 | 72.48 | 69.01 | 3.47 |
| 🥉 F2DC+DaA | **76.79** | 62.26 | 77.78 | 70.76 | 71.90 | **69.12** | **2.78** ⭐最稳 |
| ❌ **PG-DFC+DaA** | 74.85 | 62.75 | 78.42 | 70.07 | **71.52** ⚠️ | 68.07 ⚠️ | 3.45 |

#### 关键 Δ

| 对比 | AVG_B Δ | AVG_L Δ | 解读 |
|---|:--:|:--:|---|
| F2DC+DaA vs F2DC vanilla | **-1.79** ⚠️ | -0.25 | DaA 在 PACS 上 Best 角度 negative, Last 几乎 zero-sum |
| PG-DFC vanilla vs F2DC vanilla | -1.21 | -0.36 | PG-DFC 单独轻微 negative |
| PG-DFC+DaA vs F2DC vanilla | **-2.17** ⚠️ | -1.30 | 两个 negative 几乎线性叠加 (-1.21+-1.79=-3.00, 实际 -2.17) |
| PG-DFC+DaA vs F2DC+DaA | -0.38 | -1.05 | DaA 是主因, PG-DFC 仅多输 0.38 Best |

#### 重大发现 (跟 office 完全相反)

| Domain | vanilla | +DaA Last | Δ Last | DaA 角色 |
|---|:--:|:--:|:--:|:--:|
| photo (2 client, 449 sample each) | 68.12 | 74.26 | **+6.14** ⭐ | DaA 救 |
| art (3 client, 552 each) | 53.31 | 60.91 | **+7.60** ⭐⭐ | DaA 救最多 |
| cartoon (2 client, 631 each) | 75.54 | 74.36 | -1.18 | DaA 略拖 |
| **sketch (3 client, 1059 each)** | **80.51** | **66.94** | **-13.57** ⚠️ | **DaA 大杀** |
...
→ sketch 是 PACS 数据最多的 domain (单 client sample 也最多), vanilla F2DC 自然学得最好 (sketch acc 80.5%). DaA 公式按 client sample 数拉平 (大 client 降权 -31%, 小 client 升权 +51%), sketch client 主力被砍.
...
| 项 | 论文 | 我们 (sc 数据) | Δ |
|---|:--:|:--:|:--:|
| F2DC w/o DaA AVG | 75.33 | 73.69 | -1.64 |
| F2DC full AVG | 76.47 | 71.90 | -4.57 |
| F2DC vs +DaA Δ (DaA 增量) | **+1.14** | **-1.79** | **方向相反** ⚠️ |
| sketch w/o DaA | 80.13 | 82.42 | +2.29 (我们 vanilla sketch 异常高) |
| sketch w/ DaA | 82.11 | 70.76 | -11.35 (我们 +DaA sketch 大跌) |

→ 论文 +DaA 涨 sketch +1.98 (80.13→82.11), 我们 +DaA 跌 sketch -11.66 (82.42→70.76). **真正问题**: 我们 vanilla sketch 复现异常高 (+2.29 vs 论文), DaA 把过度倾向 sketch 的部分拉回 → 反向 effect.
...
| **PG-DFC + DaA (V100 s=333)** ⭐ | TBD | TBD | TBD | TBD | **63.76** (有 diag) | TBD | TBD |
| **PG-DFC + DaA 2-seed mean** | — | — | — | — | **63.78** ⭐ | — | — |
