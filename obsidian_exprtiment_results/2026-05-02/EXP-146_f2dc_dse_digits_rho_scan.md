...
`bagbb0xyq` 这个 launcher 的 `until ... done` wait 条件 (sub2 procs=0) 在低-rho 4 runs 完成后**第二次触发**, 又 fire 了一次高-rho launch (写 5 round 后被 kill)。**高-rho 4 个 R100 logs 被覆盖到 R5, log 中已无 R6-R99 数据**。 但:
- ✅ `final_R100.npz` 完整保留 (含 features + state_dict + confusion + per_domain_acc)
- ✅ `best_R0xx.npz` 完整保留 (R6+ 的 best 都在)
- ✅ `proto_logs.jsonl` 跟 `round_006-100.npz` 完好
- ✅ **per-domain best 已从 npz confusion matrix 算回准确数据** (下表全部用 npz 提)

## R100 完整 per-seed × per-domain 准确率表 (全数据从 npz)

> 每 cell = 该 run R@best round 的 4 域 acc (用 best_R0xx.npz / final_R100.npz 中 confusion matrix 对角线算: `acc = diag(cm).sum() / cm.sum()`). drift = AVG Last (R99) − AVG Best.

| rho | seed | R@best | mnist | usps | **svhn** | syn | **AVG Best** | AVG Last (R99) | drift |
|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 0.05 | 15 | R85 | 97.41 | 90.28 | **90.46** | 94.73 | **93.22** | 93.34 | +0.12 |
| 0.05 | 333 | R87 | 96.81 | 92.48 | 90.31 | 93.68 | **93.32** | 93.10 | -0.22 |
| 0.1 | 15 | R100 | 97.47 | 90.78 | 90.33 | 94.92 | **93.38** | 93.34 | -0.04 |
| 0.1 | 333 | R73 | 96.82 | 92.63 | 90.51 | 93.81 | **93.44** | 93.46 | +0.02 |
| 0.2 | 15 | R53 | 97.15 | 91.98 | **53.55** ❌ | 73.45 | **79.03** | 72.43 | -6.60 |
| 0.2 | 333 | R71 | 97.02 | 92.78 | **63.56** ❌ | 74.89 | **82.06** | 77.45 | -4.61 |
| 0.3 | 15 | R34 | 97.18 | 90.53 | **60.89** ❌ | 75.02 | **80.90** | 75.57 | -5.33 |
| 0.3 | 333 | **R98** | 96.97 | 92.58 | **89.94** ✓ recover | 94.06 | **93.39** ✓ | 93.34 | -0.05 |
| **rho=0.05 mean** | — | — | 97.11 | 91.38 | 90.39 | 94.21 | **93.27** | 93.22 | -0.05 |
| **rho=0.1 mean** | — | — | 97.15 | 91.71 | 90.42 | 94.37 | **93.41** ✓ | 93.40 | -0.01 |
| **rho=0.2 mean** | — | — | 97.09 | 92.38 | **58.56** ❌❌ | 74.17 | **80.55** | 74.94 | -5.61 |
| **rho=0.3 mean** | — | — | 97.08 | 91.56 | **75.42** (split 61/90) | 84.54 (split 75/94) | **87.15** (split 81/93) | 84.46 | -2.69 |
